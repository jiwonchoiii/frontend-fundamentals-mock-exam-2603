      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {HOUR_LABELS.map(t => {
                const left = (timeToTimelineMinutes(t) / TOTAL_TIMELINE_MINUTES) * 100;
                return (
                  <Text
                    key={t}
                    typography="t7"
              ...
              >
                ...
                >
                  {roomReservations.map((res: Reservation) => {
                    const left = (timeToTimelineMinutes(res.start) / TOTAL_TIMELINE_MINUTES) * 100;
                    const width =
                      ((timeToTimelineMinutes(res.end) - timeToTimelineMinutes(res.start)) / TOTAL_TIMELINE_MINUTES) *
                      100;
                        ...
                        {isActive && (
                          ...
                          >
                            ...
                            {res.equipment.length > 0 && (
                              <div>{res.equipment.map(equipment => EQUIPMENT_LABELS[equipment]).join(', ')}</div>
                            )}
                          </div>
                        )}
                      </div>
                  ...
                  contents={
                    <ListRow.Text2Rows
                      ...
                      bottom={`${res.date} ${res.start}~${res.end} · ${res.attendees}명 · ${
                        res.equipment.map(equipment => EQUIPMENT_LABELS[equipment]).join(', ') || '장비 없음'
                      }`}
